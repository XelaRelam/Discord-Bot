...
export const upsertUserData = async (userId: string, userData: {
  ...
  try {
    ...
    if (userData.userBanned !== undefined) {
      updateData.userBanned = userData.userBanned;
    };
    if (userData.hasAwaitedBot !== undefined) {
      updateData.hasAwaitedBot = userData.hasAwaitedBot;
    };
